...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(c => c.toLower).toList.map(x => (x._1, x._2.length)).sorted
  ...
  def wordAnagrams(word: Word): List[Word] = dictionaryByOccurrences(wordOccurrences(word)).sorted
...
//    }.toList
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagrams0(ocurrences: Occurrences): List[Sentence] = {
      if (ocurrences.isEmpty) List(List())
      else {
        //val words=dictionaryByOccurrences(ocurrences)
        for {
           comb<-combinations(ocurrences)
           words<-dictionaryByOccurrences(comb)
           remainOcc<-sentenceAnagrams0(subtract(ocurrences,comb))
